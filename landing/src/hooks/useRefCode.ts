 ...
 *     visit, persisted in localStorage under `trendex_ref_code`.
 ...
 *     Also persisted (once) in localStorage under `trendex_invited_by`
...
export function useRefCode(): { myCode: string; invitedBy: string | null } {
  ...
  useEffect(() => {
    ...
    let myCode = '';
    try {
      myCode = localStorage.getItem('trendex_ref_code') || '';
    } catch {
      /* ignore */
    }
    if (!myCode) {
      myCode = generateCode();
      try {
        localStorage.setItem('trendex_ref_code', myCode);
      } catch {
        /* ignore */
      }
    }
    ...
    try {
      ...
      if (urlRef && /^[A-Za-z0-9_-]{1,32}$/.test(urlRef)) {
        invitedBy = urlRef;
        localStorage.setItem('trendex_invited_by', urlRef);
      } else {
        invitedBy = localStorage.getItem('trendex_invited_by');
      }
    } catch {
      /* ignore */
    }
