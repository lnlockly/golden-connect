...
| `api-client.js` | thin fetch wrapper to talk to `trendex-api`'s `/api/monar/*`. Reads `MONAR_API_BASE` env. |

All money flows go through `trendex-api`. The cabinet's SQLite is for
...
3. Set env `MONAR_API_BASE=http://trendex-api/api/monar` (cluster DNS).
