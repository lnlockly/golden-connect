...
function _staticPlan(answers, profile) {
  ...
  if (stage === 'newbie') {
    return [
      ...
      { title: 'Проверить кабинет Команда', description: 'goldenConnect.to/cabinet → Команда. Посмотри сколько подключилось из 5 DM. ПОЧЕМУ: метрики = понимание что работает.', time_min: 5, category: 'learning', priority: 3, suggested_time: '21:00' },
    ];
  }
