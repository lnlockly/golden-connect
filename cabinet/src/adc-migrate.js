...
CREATE TABLE IF NOT EXISTS ad_post_templates (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    title TEXT NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS ad_monitor_sources (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    title TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ad_smart_queue (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    post_id INTEGER,
    source_id INTEGER NOT NULL,
...
function applyAdCenterSchema() {
  const db = getDb();
  try { db.exec(SCHEMA_SQL); }
  catch (e) { console.error('[adc-migrate]', e.message); }
  console.log('[adc] schema ready');
}

module.exports = { applyAdCenterSchema };
