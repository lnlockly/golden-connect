// Trendex: Team notifications — sent to inviter when referral transitions stage.
...
<i>"Привет! Рад что зашёл в Trendex. Что тебя интересует — заработок на партнёрке или запуск своей рекламы? Расскажу подробно как стартовать."</i>`;
  },
  onboarded: (ref) => {
    const contact = buildContactBlock(ref);
...
💡 <b>Шаблон:</b>
<i>"Вижу ты активно используешь Trendex! Время апнуться до BOOST/ROCKET — глубже матрица + Matching. Давай созвонимся на 10 минут?"</i>`;
  },
  converted: (ref) => {
    const contact = buildContactBlock(ref);
...
3️⃣ Добавь в чат партнёров @TRENDEX_AD
...
💡 <b>Шаблон:</b>
<i>"Привет! Давно не виделись в Trendex. На бирже появились новые задания и стартует новый челлендж недели. Хочешь посмотреть что подойдёт тебе?"</i>`;
  },
  lost: (ref) => {
    const contact = buildContactBlock(ref);
...
💡 <b>Шаблон:</b>
<i>"Привет! Вышли крупные обновления Trendex — новый Лидерский пул, апгрейд тарифа со скидкой. Если хочешь — расскажу подробнее за 5 минут."</i>`;
  }
};
