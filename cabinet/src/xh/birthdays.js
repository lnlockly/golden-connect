// Trendex Birthdays module — track birthdays of contacts and generate
  ...
  bot.callbackQuery('dr_settings', async (ctx) => {
    ...
    const T = lang === 'en' ? {
      ...
      biz: has
        ...
        : '⚠️ Business Bot not connected.\nTo enable auto-sending: Settings → Telegram Business → Chatbots → @' + (ctx.me.username || 'Trendex_bizbot'),
      ...
      biz: has
        ...
        : '⚠️ Business Bot не подключён.\nЧтобы включить авто-отправку: Настройки TG → Telegram Business → Чат-боты → @' + (ctx.me.username || 'Trendex_bizbot'),
