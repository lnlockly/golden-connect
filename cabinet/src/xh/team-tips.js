// Trendex: AI-powered daily tip for team management.
...
function buildPrompt(inviter, refs, stats, nextActions) {
  const lines = [
    'Ты — AI-ментор для партнёра проекта Trendex.',
    'Проект: Trendex — рекламная экосистема с распределённой прибылью. Платформа оплачивает внимание аудитории (до $20/день), партнёры получают % от оборота. 10 уровней реф-программы, Matching Bonus, Лидерский пул, Gift-счёт.',
    ...
    '⚫ lost — 30+ дней тишины',
    '',
    '=== ИНСТРУМЕНТЫ В БОТЕ @Trendex_bizbot ===',
    ...
    '  /hashtags — подборки хэштегов под Trendex-темы',
    '  /aipost — AI генерирует уникальный пост через Groq',
    '/team — текущая команда, воронка стадий, карточки рефералов',
    '/advice — Trendex-коуч: быстрые советы по приглашению, возражениям, тарифам',
...
async function generateTeamTip(storage, inviter, config, opts = {}) {
  ...
  if (stats.total === 0) {
    const tip = [
      ...
      '2. Отправьте 5 знакомым, которым может быть интересна тема Trendex',
