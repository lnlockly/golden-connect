// Group-level "silent mode" for Trendex bot.
...
// admin must run /trendex_active inside the group.
//
...
function setupCommands(bot) {
  // /trendex_active — admin: enable chat features in this group
  bot.command('trendex_active', async (ctx) => {
    ...
    return ctx.reply(
      ...
      'Чтобы вернуть тишину — /trendex_silent',
      { parse_mode: 'HTML' }
    );
  });

  // /trendex_silent — admin: disable chat features (back to default)
  bot.command('trendex_silent', async (ctx) => {
    if (!ctx.chat || ctx.chat.type === 'private') return ctx.reply('Эта команда работает только в группе.');
    ...
    return ctx.reply(
      ...
      'Чтобы включить чат-режим — /trendex_active',
      { parse_mode: 'HTML' }
    );
  });

  // /trendex_status — show current mode
  bot.command('trendex_status', async (ctx) => {
    if (!ctx.chat || ctx.chat.type === 'private') return ctx.reply('Эта команда работает только в группе.');
    const active = isActive(ctx.chat.id);
    return ctx.reply(
      active
        ? '✅ Режим: <b>Чат-режим включён</b>.\n\nБот реагирует на команды, упоминания, шлёт digest. /trendex_silent чтобы выключить.'
        : '🤫 Режим: <b>Тихий (по умолчанию)</b>.\n\nБот молчит в чате, делает только тех. работу (ADX, автопостинг). /trendex_active чтобы включить чат-функции.',
      { parse_mode: 'HTML' }
    );
  });
  ...
  bot.on('my_chat_member', async (ctx) => {
    try {
      ...
      // Default = silent. No welcome message. Admin sees status via /trendex_status.
...
function setupSilenceGate(bot) {
  bot.use(async (ctx, next) => {
    try {
      ...
      if (/^\/(trendex_active|trendex_silent|trendex_status)(@|\b)/.test(txt)) return next();
