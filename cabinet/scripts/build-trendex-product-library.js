...
const EXPORT_ROOT = path.join(ROOT, 'data', 'telegram-exports', 'trendex_and_beauty');
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'trendex-product-library.json');
...
function getProductFamily(productId) {
  ...
  return families[productId] || 'Продуктовая линия Trendex';
}
...
function main() {
  ...
  const payload = {
    ...
    links: {
      ...
      officialSite: siteContent.links && (siteContent.links.companyMain || siteContent.links.officialSite) || 'https://trendex.biz/',
      companyCatalog: siteContent.links && (siteContent.links.companyCatalog || siteContent.links.shop) || 'https://trendex.biz/',
      bot: 'https://t.me/Trendex_bizbot',
    },
    summary: {
