...
function fetchUrlContent(url, timeout = 5000) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { timeout, headers: { 'User-Agent': 'Arsenal Profi-LinkPreview/1.0' } }, (res) => {
...
router.post('/links', authRequired, (req, res) => {
  try {
    ...
    const _chkSh = require('../helpers/usage-limits').checkLimit(req.user.id, 'shortener.create');
...
const BIO_PAGE_LIMITS = { free: 1, starter: 3, pro: 10, agency: 30 };
...
function createPromoLinksForUser(db, userId) {
  try {
    ...
    for (const svc of PROMO_SERVICE_IDS) {
      ...
      const refUrl = 'https://arsenalprofi.ru/?ref=' + user.ref_code + '&utm_source=promo&utm_campaign=' + svc;
      const result = db.prepare(
...
router.post('/bio/pages', authRequired, (req, res) => {
  try {
    const db = getDb();
...
router.post('/bio/pages/:id/clone', authRequired, (req, res) => {
  try {
    const db = getDb();
    const plan = _bioGetUserPlan(req.user.id);
    const limit = BIO_PAGE_LIMITS[plan] || 1;
...
router.get('/bio/pages/:id/qr', authRequired, (req, res) => {
  try {
    ...
    const QRCode = require('qrcode');
    const url = 'https://app.arsenalprofi.com/bio/' + page.username;
...
router.post('/bio/ai-generate', authRequired, async (req, res) => {
  try {
    ...
    const _chkBioGen = require('../helpers/usage-limits').checkLimit(userId, 'ai.bio-gen');
    ...
    // Get Groq key (reuse rotation from wp-api.js)
    // Get Groq key using rotation (same pattern as wp-api.js)
    let groqApiKey;
    try {
      let gk = db.prepare(`SELECT api_key FROM groq_keys WHERE is_active = 1
        AND (last_used_at IS NULL OR last_used_at < datetime('now', '-48 hours'))
        ORDER BY last_used_at ASC, total_uses ASC LIMIT 1`).get();
      if (!gk) gk = db.prepare(`SELECT api_key FROM groq_keys WHERE is_active = 1
        ORDER BY total_uses ASC, last_used_at ASC LIMIT 1`).get();
      if (gk) {
        groqApiKey = gk.api_key;
        db.prepare("UPDATE groq_keys SET last_used_at = datetime('now'), total_uses = total_uses + 1 WHERE api_key = ?").run(groqApiKey);
      }
    } catch(e) {}
    if (!groqApiKey) groqApiKey = process.env.GROQ_API_KEY;
    if (!groqApiKey) {
...
router.post('/bio/pages/:id/domain/:did/verify', authRequired, async (req, res) => {
  try {
    ...
    try {
      ...
      if (cname.some(c => c.includes('arsenalprofi.com') || c.includes('81.91.177.204'))) {
        verified = true;
      }
    } catch(e) {
    ...
    if (!verified) {
      try {
        ...
        if (addresses.includes('81.91.177.204')) {
          verified = true;
        }
      } catch(e) {
    ...
    if (verified) {
      ...
      let sslOk = false;
      try {
        execSync('certbot certonly --nginx -d ' + domainRow.domain + ' --non-interactive --agree-tos -m admin@arsenalprofi.com 2>&1', { timeout: 60000 });
        sslOk = true;
      } catch(e) {
        // SSL might fail if certbot limit reached
      }

      if (sslOk) {
        // Create nginx config
        const nginxConf = _generateNginxConf(domainRow.domain, domainRow.bio_id);
        ...
        try {
          ...
          db.prepare("UPDATE bio_custom_domains SET ssl_status = 'active' WHERE id = ?").run(domainRow.id);
        } catch(e) {
          // Remove bad config
          try { fs.unlinkSync(confPath); } catch(e2) {}
          db.prepare("UPDATE bio_custom_domains SET ssl_status = 'error', error_message = 'Nginx config error' WHERE id = ?").run(domainRow.id);
        }
      } else {
        db.prepare("UPDATE bio_custom_domains SET ssl_status = 'pending', error_message = 'SSL pending' WHERE id = ?").run(domainRow.id);
      }
      ...
      res.json({ success: false, verified: false, error: errorMsg || 'DNS not pointing to app.arsenalprofi.com. Add CNAME or A record.' });
    }
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
