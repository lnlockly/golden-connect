...
const MEDIA_UPLOAD_DIR = path.join('/opt/banner-webapp', 'public', 'uploads', 'ad-media');
...
router.delete('/media/:id', authRequired, (req, res) => {
  try {
    ...
    if (m.url && m.source === 'upload') {
      const fpath = path.join('/opt/banner-webapp/public', m.url);
...
router.post('/schedules/:id/run-now', authRequired, (req, res) => {
  try {
    ...
    const cronPath = require('path').join(__dirname, '../../scripts/ad-center-cron.js');
...
router.post('/video/compose-post', authRequired, async (req, res) => {
  ...
  try {
    ...
    if (banner_id) {
      ...
      // Найти баннер-видео
      const bannerFile = path.join('/opt/banner-webapp/public/generated/banners', banner_id);
    ...
    const fullMediaUrl = 'https://app.arsenalprofi.com' + finalVideoUrl;
...
router.get('/video/banners', authRequired, (req, res) => {
  try {
    const bannerDir = '/opt/banner-webapp/public/generated/banners';
...
router.post('/monitors', authRequired, (req, res) => {
  const db = getDb();
