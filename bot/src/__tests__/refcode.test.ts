...
// data layer became thin HTTP → goldenConnect-api. Re-add them as integration
...
test("refcode: invite + website links embed the tg_id-derived ref_code", () => {
  ...
  assert.equal(
    ...
    "https://goldenConnect.website/?ref=1361064246",
  );
  assert.equal(
    buildWebsiteLink("https://goldenConnect.website/", code),
    "https://goldenConnect.website/?ref=1361064246",
...
test("migration 002: legacy random ref_codes are rewritten to tg_ids", { skip: "moved to goldenConnect-api" }, () => {});
test("repo: listAncestors walks the ref chain in depth-ascending order", { skip: "TODO: rewrite as api-client mock / integration" }, () => {});
test("repo: dashboard computes growth projection from 7d window", { skip: "moved to goldenConnect-api" }, () => {});
