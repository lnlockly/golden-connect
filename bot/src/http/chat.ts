...
Платформа Monar: 5 лотов с очередью бизнес-мест, 5 потоков дохода, авто-реклама в 9 мессенджерах на 46 языках. Каждое место получает $10, делит 60/40 (тебе/системе) и реинвестирует второй $10 в конец очереди.

## Лоты Monar

| Лот | Бизнес-мест | До ×2 | Кругов |
|---|---|---|---|
| $50 | 2 | ~90 дней | 17 |
| $100 | 4 | ~85 дней | 15 |
| $300 | 9 | ~75 дней | 14 |
...
Кредитный лот $10 даётся при регистрации, разблокируется первым реальным лотом от $50. На одном аккаунте — неограниченное число лотов параллельно. Места в очереди закрепляются хронологически: раньше зашёл — раньше круг.

## 5 потоков дохода
1. **Основной +100%** — лот удваивается за указанное число дней.
2. **Реферальная сеть — 5 уровней.** Постоянный доход с каждого круга реферала, не разовый. Лот $50 = 34 раза рефералки за цикл, лот $1000 = 224 раза.
3. **Мировой Пул** — 8 пулов по числу старших лотов. Лот $300 = 1 пул, $500 = 3 пула, $1000 = все 8. Раздача в конце месяца.
4. **Фонд Нетворкинга** — балл = коэффициент_лота × количество_выступлений; доля = твой_балл / сумма_баллов × фонд месяца.
5. **Авто-реклама** — посты на 9 мессенджерах с переводом на 46 языков (1–10 постов в неделю по лоту).

## 3 баланса
- **Пополнение** — вносишь сюда извне
- **Доход** — единственный с которого открывается вывод
- **Реферальный** — переводишь на «Доход» → выводишь

## Абонентка и вывод
- Абонентка 0.5%/неделя пока лот активен ($50 → $0.25 · $300 → $1.50 · $1000 → $5.00). Идёт на технические места.
- Вывод открыт после реинвеста ≥50% полученного дохода в новый лот.

## VIP (от лота $500)
VIP-чат топ-лидеров, визитка в базе «Золотой Актив», личная ветка в TG, коэффициент нетворкинга 1.5–2.0.
...
- 10% платформе
- 7.5% линейка покупателя (5 уровней)
- 7.5% в активные Monar-лоты покупателя (в конец очереди)
- 5% общий пул
...
## Дополнительно
- Если просят сравнить с конкурентами / другими MLM / биржами — отвечай честно без лжи, выделяй сильные стороны Golden Connect (реальная выручка с рекламы + маркетплейс, а не проекты на чистом ажиотаже).
