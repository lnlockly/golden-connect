...
export async function handleModerationAlert(
	...
	const kb = new InlineKeyboard()
		...
		.row()
		.url("📋 Открыть в CRM", `https://crm.goldenConnect.to/cabinet/crm-app.html#/moderation/${campaignId}`)
...
export async function handleBillingAlert(
	...
	// Resolve user's TG chat_id via goldenConnect-api internal.
	let chatId: number | null = null
	try {
		const apiBase = (process.env.GOLDEN_CONNECT_API_INTERNAL_URL || "https://api.goldenConnect.to/internal").replace(/\/internal\/?$/, "")
		const r = await fetch(
			apiBase + `/internal/users/by-id/${userId}`,
			{ headers: { "x-goldenConnect-secret": deps.internalSecret } }
		)
		if (r.ok) {
...
export async function handleCrmInbound(
	...
	const html =
		...
		`<a href="https://goldenConnect.to/cabinet/crm-app.html">→ Открыть в CRM</a>`
	const kb = new InlineKeyboard().url("📂 Открыть чат в CRM", "https://goldenConnect.to/cabinet/crm-app.html")
	try {
		await deps.bot.api.sendMessage(chatId, html, { parse_mode: "HTML", reply_markup: kb })
	} catch (e) {
