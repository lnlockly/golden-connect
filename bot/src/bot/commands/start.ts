...
function _buildCrmMenu(name: string, tariff: CrmTariffInfo, webappUrl: string): {
  ...
  const text =
    ...
    `\n` +
    `📋 База 7 322 партнёров и лидеров готова. Веди их в воронке, ставь задачи, отправляй AI-питчи.\n` +
    `\n` +
    `<b>Главные команды:</b>\n` +
...
function _buildCrmPaywall(name: string, tariff: CrmTariffInfo, webappUrl: string): {
  ...
  const text =
    ...
    `\n` +
    `📋 Здесь у тебя будет:\n` +
    `• База 7 322 партнёров и лидеров\n` +
    `• AI-питчи под каждый контакт\n` +
    `• Воронка сделок + задачи\n` +
    `• Уведомления «кому позвонить сегодня»\n` +
    `\n` +
    `🔒 CRM открывается с любым активным лотом Monar:\n` +
    `• <b>$50</b> — 2 бизнес-места, ×2 за ~90 дней\n` +
    `• <b>$300</b> — 9 мест + вход в Мировой Пул\n` +
    `• <b>$1000</b> — 32 места, все 8 пулов, VIP-чат\n` +
    `\n` +
    statusLine + `\n` +
    `\n` +
    `Нажми «Активировать лот» — внутри кабинета выберешь любой и оплатишь картой или криптой.`;
  const kb = new InlineKeyboard()
    .webApp('💎 Активировать лот', buyUrl)
    .row()
