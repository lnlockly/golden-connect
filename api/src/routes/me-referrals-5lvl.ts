...
import { MAX_REFERRAL_LEVEL, REFERRAL_CURVE_5LVL } from '../services/referrals-5lvl.js';

const app = new Hono<{ Variables: AuthVars }>();

app.use('/me/referrals/5lvl', requireAuth);
...
/**
 * GET /me/referrals/5lvl — five-row breakdown of the caller's classic
 ...
 * source of truth for ref_L1..L5 credits and is paired 1:1 with the
 * matching `cash_ledger` rows by the accrual writer.
 */
app.get('/me/referrals/5lvl', async (c) => {
  ...
  for (let lvl = 1; lvl <= MAX_REFERRAL_LEVEL; lvl++) {
    ...
    levels.push({
      ...
      pct: REFERRAL_CURVE_5LVL[lvl - 1] ?? 0,
    });
  }
