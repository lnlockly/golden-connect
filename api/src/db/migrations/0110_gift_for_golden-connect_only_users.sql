-- =====================================================================
-- 0110: GIFT-аккаунты для чистых Golden Connect-юзеров (reverse-link)
-- =====================================================================
-- 10 golden-connect users без gift_user → создаём gift_users (0 балансы),
-- чтобы у каждого Golden Connect-юзера была единая GIFT-идентичность.
...
ins AS (
  INSERT INTO gift_users (
    ...
    golden-connect_user_id, migrated_at, created_at, updated_at
  )
  SELECT a.max_gc + t.rn,
         'golden-connect-' || t.uid,
         'user',
         COALESCE(NULLIF(t.first_name, ''), 'Golden ConnectUser'),
  ...
  RETURNING id, golden-connect_user_id
)
UPDATE users u SET gift_user_id = ins.id
FROM ins WHERE u.id = ins.golden-connect_user_id;

-- нулевые строки Текущего (5) и Основного (6) для свежесозданных gift-юзеров
-- (точное совпадение uuid = 'golden-connect-<users.id>' — не задеваем старые golden-connect-* аккаунты)
...
WHERE gu.golden-connect_user_id IS NOT NULL
  AND gu.uuid = 'golden-connect-' || gu.golden-connect_user_id::text
  AND NOT EXISTS (
