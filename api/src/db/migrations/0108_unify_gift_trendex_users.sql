-- =====================================================================
-- 0108: Унификация gift ↔ trendex юзеров (часть A — создание юзеров)
-- =====================================================================
-- Каждый ГЛАВНЫЙ gift-аккаунт (main_user_id IS NULL) должен иметь
-- соответствующего trendex users-юзера, чтобы работало пополнение
-- Основного баланса (списание с working) и единая идентичность.
--
-- Решения пользователя:
--   - 1 trendex-юзер на telegram tg_id; дубли главных по tg → схлопнуть
...
-- Идемпотентно: создаём только для trendex_user_id IS NULL.
-- =====================================================================
...
-- Шаг 1: создать trendex users для непривязанных главных + проставить связь
WITH to_create AS (
  ...
  FROM gift_users gu
  WHERE gu.main_user_id IS NULL
    AND gu.trendex_user_id IS NULL
